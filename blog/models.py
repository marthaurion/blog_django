...
def comment_import():
    ...
    for post_id in parsed:
        email = parsed[post_id]["email"]
        name = parsed[post_id]["name"]
        existing = Commenter.objects.filter(email=email)
        if not existing:
            commenter = Commenter()
            commenter.email = email
            commenter.username = name
            commenter.save()
        else:
            commenter = existing[0]
            
        post_set = Post.objects.filter(slug=parsed[post_id]["thread"])
        if not post_set:
            continue
        post = post_set[0]
        comment = Comment()
        comment.author = commenter
        if "parent" in parsed[post_id]:
            parent = parsed[post_id]["parent"]
            if parent in post_map:
                comment.parent = post_map[parent]
        comment.pub_date = datetime.strptime(parsed[post_id]["post_date"], "%Y-%m-%dT%H:%M:%SZ").replace(tzinfo=timezone.utc)
        comment.post = post
        comment.text = parsed[post_id]["message"]
        comment.save()
        post_map[post_id] = comment